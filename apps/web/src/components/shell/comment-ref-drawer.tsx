...
/** Drawer genérico para listas de comentários referenciados (Assigned / Replies). */
export function CommentRefDrawer({
  title,
  kind,
  items,
  onClose,
  ...
  title: string;
  kind: "assigned" | "replies";
  items: CommentRef[];
  onClose: () => void;
}) {
  const router = useRouter();
  const [pending, startTransition] = React.useTransition();
  const Icon = kind === "replies" ? Reply : MessageSquare;
    ...
    <div className="fixed inset-0 z-40 flex justify-end bg-dark/50 animate-fade-in" onClick={onClose}>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {items.length === 0 ? (
            <p className="p-4 text-center text-ui text-muted">
              {kind === "replies"
                ? "Ninguém respondeu seus comentários ainda."
                : "Nenhum comentário atribuído a você."}
            </p>
          ) : (
            items.map((c) => (
              <button
